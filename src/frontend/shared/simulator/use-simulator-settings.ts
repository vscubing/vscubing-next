...
export function useMutateSimulatorSettings() {
  ...
  function handleMutation(newSettings: Partial<SimulatorSettings>) {
    ...
    debounceSettings(async () => {
      try {
        await mutateAsync({ ...oldSettings, ...newSettings }) // NOTE: we have to merge oldSettings with newSettings so that cameraPosition and puzzleSize changes don't override each other in a race condition
      } catch {
