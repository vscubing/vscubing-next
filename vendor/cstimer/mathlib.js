...
const mathlib = (function () {
  ...
  schreierSims.prototype.minkwitz = function () {
    ...
    var iterGens = function (p, remain, func) {
      ...
      for (var i = 0; i < this.gen.length && toFill > 0; i++) {
        words.push(i)
        var ret = iterGens.call(
          this,
          this.permMult(p, this.gen[i]),
          remain - 1,
          func,
        )
        words.pop()
        ...
        iterGens.call(
          this,
          this.permMult(p, this.permInv(this.gen[i])),
          remain - 1,
          func,
        )
        words.pop()
        ...
        for (var j1 = 0; j1 < i1; j1++) {
          ...
          for (var i2 = i1; i2 < this.e.length; i2++) {
            ...
            for (var j2 = i1 == i2 ? j1 : 0; j2 < i2; j2++) {
              ...
              if (
                this.words[i1][cc] &&
                this.words[i1][cc].length < cuml * 1.5 &&
                i1 != i2
              ) {
                continue
              }
              var ret = addWords.call(
                this,
  ...
  schreierSims.prototype.enumDFS = function (depth, perm, callback, checkFunc) {
    ...
    for (var j = 0; j <= depth; j++) {
      if (this.sgs[depth][j]) {
        var ret = this.enumDFS(
          depth - 1,
          this.permMult(this.sgs[depth][j], perm),
          callback,
          checkFunc,
        )
        if (ret) {
  ...
  _.calcNumOfStates = function () {
    ...
    for (var moveIdx = 0; moveIdx < this.movesList.length; moveIdx++) {
      ...
      var newState = this.doMove(
        String.fromCharCode.apply(null, state),
        this.movesList[moveIdx][0],
      )
